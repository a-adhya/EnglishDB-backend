...
Your response should be only the SQL query to execute. Reference the following schema: """ + read_schema_file("tests/media/media_schema.sql")
...
def execute_query(query):
    ...
    conn = sqlite3.connect("var/chinook.sqlite3")
    c = conn.cursor()
    c.execute(query)
    rows = c.fetchall()
    conn.close()
...
def run_threaded_tests():
    ...
    questions = read_questions_file("tests/media/media.txt")
    print("*************************TESTS****************************\n\n")
    threads = []
    #run thread
